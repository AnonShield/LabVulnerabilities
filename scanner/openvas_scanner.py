...
class Config:
    ...
    # Relatórios - formatos a baixar
    report_formats: list = field(default_factory=lambda: ["PDF", "XML", "CSV", "TXT"])
...
class ScanResult:
    ...
    reports_downloaded: list = field(default_factory=list)
...
class GVMClient:
    ...
    def wait_for_gvm(self, max_wait: int = 600) -> bool:
        ...
        while time.time() - start < max_wait:
            try:
                ...
                self.logger.warning("GVM ainda inicializando (connection reset). Aguardando 30s...")
                time.sleep(30)
    ...
    def _cache_ids(self, gmp):
        ...
        for config in configs.findall(".//config"):
            ...
            if name == self.config.scan_config_name:
                self._cache["scan_config_id"] = config.get("id")
                break

        if "scan_config_id" not in self._cache:
            # Tentar encontrar qualquer config "Full and fast"
            for config in configs.findall(".//config"):
                name = config.find("name").text
                if "full and fast" in name.lower():
                    self._cache["scan_config_id"] = config.get("id")
                    self.logger.info(f"Usando scan config: {name}")
                    break
        ...
        for pl in port_lists.findall(".//port_list"):
            ...
            if self.config.port_list_name.lower() in name.lower():
                self._cache["port_list_id"] = pl.get("id")
                break

        if "port_list_id" not in self._cache:
            # Usar primeira port list disponível
            first_pl = port_lists.find(".//port_list")
            if first_pl is not None:
                self._cache["port_list_id"] = first_pl.get("id")
                self.logger.info(f"Usando port list: {first_pl.find('name').text}")

        # Scanners
        scanners = gmp.get_scanners()
        for scanner in scanners.findall(".//scanner"):
            name = scanner.find("name").text
            if "openvas" in name.lower() and "default" in name.lower():
                self._cache["scanner_id"] = scanner.get("id")
                break

        if "scanner_id" not in self._cache:
            for scanner in scanners.findall(".//scanner"):
                name = scanner.find("name").text
                if "openvas" in name.lower():
                    self._cache["scanner_id"] = scanner.get("id")
                    self.logger.info(f"Usando scanner: {name}")
                    break
        ...
        self.logger.debug(f"Cache: {self._cache}")
